  ...
  while (!done) {
    ...
    for (const rawtx of txs) {
      ...
      if (rawtx.status === 'completed') {
        const duplicate = ssFormatTxs.find(
          object => object.inputTXID === rawtx.cryptoTransactionId
        )
        if (duplicate !== undefined) {
          console.log('DUPLICATE TRANSACTION ID FOUND')
          console.log('DUPLICATE IN ARRAY:', JSON.stringify(duplicate))
          console.log('CURRENT RAW TX:', JSON.stringify(rawtx))
        }
